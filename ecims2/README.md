...
## Phase 5: Data encryption at rest (AES-256-GCM)

ECIMS encrypts sensitive server-side storage using AES-256-GCM with purpose-specific keys derived from an offline-provisioned master key via HKDF-SHA256.

Encrypted scopes:
- AI model artifacts (`ai_artifacts`)
- Encrypted audit export (`scripts/export_audit_encrypted.py`)
- Encrypted archive payload helper (`scripts/encrypt_archive.py`)
...
Signed policy controls:
- `data_encryption_required` (STRICT default `true`)
- `allow_plaintext_exports` (STRICT default `false`)
- `allow_key_rotation_grace` (STRICT default `false`)

Startup behavior:
- STRICT/fail-closed: if encryption is required and keyring is missing/invalid, startup aborts.
- Audit actions include: `DATA_ENCRYPTION_ENABLED`, `DATA_KEY_MISSING`, `DATA_KEY_INVALID`, `DATA_KEY_ROTATED`, `DATA_DECRYPT_FAILED`.

Offline key provisioning:

```bash
cd license_authority
python generate_data_key.py --out ../ecims2/server/.ecims_state/data_keyring.json --key-id key-001
```

Rotation:

```bash
python rotate_data_key.py --keyring ../ecims2/server/.ecims_state/data_keyring.json --new-key-id key-002
```
...
```bash
cd ecims2
PYTHONPATH=server python scripts/export_audit_encrypted.py
```

Operational warning:
- Backup keyring material securely offline. If keys are lost, encrypted data is unrecoverable.
